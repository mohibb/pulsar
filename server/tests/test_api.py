...
def test_portfolio_buy_deducts_cash(client):
    body = client.post("/api/portfolio/buy", json={"coin_id": "bitcoin", "usd_amount": 1000}).json()
    assert body["cash"] == pytest.approx(9_000.0)
    assert len(body["holdings"]) == 1
    assert body["holdings"][0]["coin_id"] == "bitcoin"


def test_portfolio_buy_records_transaction(client):
    client.post("/api/portfolio/buy", json={"coin_id": "bitcoin", "usd_amount": 500})
    txns = client.get("/api/portfolio").json()["transactions"]
    assert len(txns) == 1
    assert txns[0]["type"] == "buy"
    assert txns[0]["total"] == 500


def test_portfolio_buy_twice_averages_price(client):
    client.post("/api/portfolio/buy", json={"coin_id": "bitcoin", "usd_amount": 1000})
    client.post("/api/portfolio/buy", json={"coin_id": "bitcoin", "usd_amount": 1000})
...
def test_portfolio_buy_insufficient_cash_returns_400(client):
    assert (
        client.post(
            "/api/portfolio/buy", json={"coin_id": "bitcoin", "usd_amount": 99_999}
        ).status_code
        == 400
    )


def test_portfolio_buy_below_minimum_returns_400(client):
    assert (
        client.post(
            "/api/portfolio/buy", json={"coin_id": "bitcoin", "usd_amount": 0.5}
        ).status_code
        == 400
    )


def test_portfolio_buy_unknown_coin_returns_404(client):
    assert (
        client.post(
            "/api/portfolio/buy", json={"coin_id": "dogecoin", "usd_amount": 100}
        ).status_code
        == 404
    )


def test_portfolio_sell_reduces_holding(client):
    client.post("/api/portfolio/buy", json={"coin_id": "bitcoin", "usd_amount": 1000})
    body = client.post("/api/portfolio/sell", json={"coin_id": "bitcoin", "usd_amount": 500}).json()
    assert body["cash"] == pytest.approx(9_500.0)


def test_portfolio_sell_full_position_removes_holding(client):
    client.post("/api/portfolio/buy", json={"coin_id": "bitcoin", "usd_amount": 1000})
    body = client.post(
        "/api/portfolio/sell", json={"coin_id": "bitcoin", "usd_amount": 1000}
    ).json()
    assert body["holdings"] == []


def test_portfolio_sell_no_holding_returns_400(client):
    assert (
        client.post(
            "/api/portfolio/sell", json={"coin_id": "bitcoin", "usd_amount": 100}
        ).status_code
        == 400
    )


def test_portfolio_sell_over_value_returns_400(client):
    client.post("/api/portfolio/buy", json={"coin_id": "bitcoin", "usd_amount": 100})
    assert (
        client.post(
            "/api/portfolio/sell", json={"coin_id": "bitcoin", "usd_amount": 99_999}
        ).status_code
        == 400
    )


def test_portfolio_reset(client):
    client.post("/api/portfolio/buy", json={"coin_id": "bitcoin", "usd_amount": 1000})
    body = client.post("/api/portfolio/reset").json()
    assert body["cash"] == 10_000.0
    assert body["holdings"] == []
